...
module.exports = {
  ...
  async execute(interaction, client) {
    // Get type to reload from the "type" option
    const commandType = interaction.options.getString('type');

    try {
      if (commandType === 'slash') await reloadAllCommands(client.slashCommands, path.join(__dirname, '../../slash'), interaction, 'slash');
      else if (commandType === 'prefix') await reloadAllCommands(client.prefixCommands, path.join(__dirname, '../../prefix'), interaction, 'prefix');
...
async function reloadAllCommands(commandsCollection, directory, interaction, type) {
  ...
  // Read commands from the directory for the specified type
