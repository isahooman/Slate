...
module.exports = {
  ...
  async execute(message) {
    try {
      ...
      logger.debug(`Fetching original message`);
      const originalMessage = await message.channel.messages.fetch(message.reference.messageId);
      logger.debug(`Fetched original message successfully`);
      ...
      logger.debug('Replied with raw information');
    } catch (error) {
      logger.error(`Error in 'raw' command: ${error.message}`);
