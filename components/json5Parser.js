...
function readJSON5(filePath) {
  try {
    ...
    stderr.write(`Error reading JSON5 file at ${filePath}: ${error.message}\n`);
    throw error;
  }
}

/**
 * Write data to a JSON5 file.
 * @param {string} filePath Path to the JSON5 file.
 * @param {object} data Data to write to the file.
 */
function writeJSON5(filePath, data) {
  try {
    const json5Data = JSON5.stringify(data, null, 2);
    fs.writeFileSync(filePath, json5Data, 'utf-8');
  } catch (error) {
    stderr.write(`Error writing JSON5 file at ${filePath}: ${error.message}\n`);
    throw error;
  }
}
